"""
The assignment is based on an experiment designed in the 1950s by the mathematician (and juggler)
Claude Shannon 1
to examine the information content of English. The experiment was similar to the game
of Hangman, where players attempt to reveal a hidden word by guessing which letters are in it. However,
instead of guessing single words, subjects were asked to guess entire phrases they had never seen before,
and had to guess each letter sequentially.
The number of attempts it took for subjects to correctly guess each letter is interesting because people
don’t guess randomly. Each additional letter that is revealed allows people to make better predictions
about what will come next—even if they are unfamiliar with the word or phrase. For example, if you
know that the first two letters of a word are “TH”, then you will probably be able to make a pretty good
guess for the most likely possibilities for the next letter.2
Although this may seem like just an interesting curiosity, the game has a very important application:
predicting the next character of text or pixel in an image is the basis of compression systems, and the
better the prediction is made, the better the compression that can be achieved. This idea underpins
many compression methods that are now commonplace, including zip, rar, bzip, jpeg, mp3 and mpeg
compression.
"""
...
def select_next_guess(possibles):
    ...
    if possibles.head is None:                                          # If the given list is empty
        ...
        current_highest = possibles.head                                # make the head item the highest frequency
        ...
        return current_highest                                          # and return it
