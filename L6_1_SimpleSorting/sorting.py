...
def insertion_sort(file_name='file3.txt'):
    """ Loads numbers from the given file and returns them as a sorted list.
...
def shell_sort(file_name='file3.txt'):
    """
    ...
    while gap > 0:
        ...
        for start_position in range(gap):
            sub_comparisons = gap_insertion_sort(alist, start_position, gap)
            total_comparisons += sub_comparisons
        gap = gap // 2
    # Note: you will need to count the comparisons
