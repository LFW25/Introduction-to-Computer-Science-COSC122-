...
class BinarySearchTree(object):
    ...
    def _pop_min_recursive(self, subtree_root):
        ...
        if subtree_root.left.left is None:
            successor = subtree_root.left
            if successor.right is not None:
                subtree_root.left = successor.right
            else:
                subtree_root.left = None
            return successor.value
        else:
            min_val = self._pop_min_recursive(subtree_root.left)
            return min_val
...
if __name__ == '__main__':
    ...
    file = load_file('list2.txt')
    bst = BinarySearchTree()
    for item in file:
        bst.insert(item)
    print(bst._pop_in_order_successor(bst.root))